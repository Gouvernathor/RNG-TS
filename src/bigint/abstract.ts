...
export function makeRandom({
    ...
    const numberBitsOfNext = Number(nBitsOfNext);

    const getNumberOfNextCalls = numberBitsOfNext % 4 === 0 ?
        (resolution: bigint) => {
            ...
            return Math.ceil(nBits4 / numberBitsOfNext);
        } :
        (resolution: bigint) => {
            const nBits = resolution.toString(2).length;

            return Math.ceil(nBits / numberBitsOfNext);
        };
    ...
    return function random (this: AbstractBigIntRNG, resolution = this.resolution) {
        ...
        const numberOfNextCalls = getNumberOfNextCalls(resolution)
        ...
        const numerator = values.reduce((prev, curr) => (prev << nBitsOfNext) + curr);

        // the denominator is 2** (64 * the number of times #next was called)
        const denominator = 2n ** (nBitsOfNext * BigInt(numberOfNextCalls));
